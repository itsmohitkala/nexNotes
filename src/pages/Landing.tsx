      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 h-16">
          <Link to="/" className="flex items-center gap-2.5">
            <img src="/logo.png" alt="NexNotes" className="h-8 w-8 rounded-lg" />
            <span className="text-base font-bold text-foreground tracking-tight">NexNotes</span>
          </Link>
          ...
          <div className="flex items-center gap-2.5">
            ...
            {user ? (
              <>
                <Link to="/workspace">
                  <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground text-sm font-medium">Workspace</Button>
                </Link>
                <Button variant="ghost" size="sm" onClick={signOut} className="text-muted-foreground hover:text-foreground text-sm font-medium">Log out</Button>
              </>
            ) : (
              <Link to="/login">
                <Button size="sm" className="text-sm bg-brand text-brand-foreground hover:bg-brand-dark rounded-lg font-medium h-9 px-5">Sign in</Button>
              </Link>
            )}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6 pt-20 pb-24 md:pt-28 md:pb-32">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <motion.div initial="hidden" animate="visible" className="space-y-8">
              <motion.h1 variants={fadeUp} custom={0} className="text-5xl md:text-[3.75rem] font-bold tracking-tight text-foreground leading-[1.08]">
                Turn Any Document<br />Into Smart AI Notes
              </motion.h1>
              <motion.p variants={fadeUp} custom={1} className="text-muted-foreground text-base md:text-lg leading-relaxed max-w-[460px]">
              ...
              <motion.div variants={fadeUp} custom={2} className="flex items-center gap-3 pt-1">
                <Button
                  size="lg"
                  className="bg-brand text-brand-foreground hover:bg-brand-dark rounded-xl px-8 h-13 text-base font-semibold shadow-lg shadow-brand/25"
                  onClick={handleCTA}
                >
                ...
                <Button
                  variant="outline"
                  size="lg"
                  className="rounded-xl px-8 h-13 text-base font-semibold border-border text-foreground hover:bg-accent"
                  onClick={() => document.getElementById('how-it-works')?.scrollIntoView({ behavior: 'smooth' })}
                >
                  See Demo
                </Button>
              </motion.div>
              <motion.p variants={fadeUp} custom={3} className="text-sm text-muted-foreground tracking-wide">
            ...
            >
              <div className="rounded-2xl border border-border bg-card shadow-2xl shadow-brand/5 overflow-hidden">
                <div className="flex items-center gap-1.5 px-4 py-3 border-b border-border bg-accent/40">
                  ...
                  <span className="ml-3 text-[11px] text-muted-foreground font-medium tracking-wide">NexNotes</span>
                </div>
                <div className="flex min-h-[320px]">
                  {/* Sidebar */}
                  <div className="w-[100px] border-r border-border p-3.5 space-y-2.5 bg-accent/20 shrink-0">
                    <div className="text-[11px] font-semibold text-foreground mb-3 tracking-wide">Your Notes</div>
                    {['Research Paper', 'Biology Ch.4', 'Meeting Notes'].map((t, i) => (
                      <div key={t} className={`text-[11px] px-2.5 py-2 rounded-lg truncate ${i === 0 ? 'bg-brand/10 text-brand border border-brand/20 font-medium' : 'text-muted-foreground'}`}>
                        {t}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex-1 p-4 space-y-3.5 border-r border-border">
                    <div className="text-[11px] font-semibold text-foreground tracking-wide">AI-Generated Notes</div>
                      ...
                      {[
                        ...
                        <div key={s.label} className="flex items-start gap-2.5">
                          ...
                          <div className="space-y-1.5 flex-1">
                            <div className={`text-[10px] font-semibold ${s.textColor} uppercase tracking-widest`}>{s.label}</div>
                            <div className={`h-2 ${s.w1} rounded-full bg-muted`} />
                            <div className={`h-2 ${s.w2} rounded-full bg-muted`} />
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="flex items-center gap-2 pt-1">
                      <div className="text-[9px] px-2.5 py-1 rounded bg-brand/10 text-brand font-semibold">Explain</div>
                      <div className="text-[9px] px-2.5 py-1 rounded bg-accent text-muted-foreground font-medium">Simplify</div>
                      <div className="text-[9px] px-2.5 py-1 rounded bg-accent text-muted-foreground font-medium">Ask AI</div>
                    </div>
                  </div>
                  {/* AI Assistant Panel */}
                  <div className="w-[150px] shrink-0 flex flex-col bg-background border-l border-border">
                    ...
                    <div className="px-2.5 pb-3 pt-2 border-t border-border">
                      <div className="flex items-center gap-2 bg-card rounded-xl border border-border px-3 py-2">
                        <span className="text-[8px] text-muted-foreground flex-1">Ask a question...</span>
          ...
          <div className="grid md:grid-cols-3 gap-7">
            {[
              ...
              >
                ...
                <h3 className="text-base font-semibold text-foreground mt-5">{item.title}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed max-w-[240px] mt-2.5 min-h-[44px]">{item.desc}</p>
          ...
          >
            <div className="rounded-2xl border border-border bg-card p-8 space-y-5">
              <div className="inline-flex items-center gap-2.5">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-destructive/70">Before</span>
              </div>
            ...
            <div className="rounded-2xl border border-brand/20 bg-card p-8 space-y-5">
              <div className="inline-flex items-center gap-2.5">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-brand">After — AI Notes</span>
              </div>
              <div className="space-y-5">
                {[
                  ...
                  <div key={s.label} className="flex items-start gap-3">
                    <div className={`w-1 rounded-full ${s.color} self-stretch shrink-0`} />
                    <div>
                      <div className={`text-[11px] font-semibold ${s.textColor} mb-1.5 uppercase tracking-widest`}>{s.label}</div>
      ...
      <section className="border-t border-border bg-accent/20">
        <div className="max-w-3xl mx-auto px-6 py-28 md:py-32 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} className="space-y-8">
            <motion.h2 variants={fadeUp} className="text-3xl md:text-5xl font-bold text-foreground tracking-tight leading-tight">
              Start Turning Documents<br />Into Smart Notes
            </motion.h2>
            <motion.div variants={fadeUp} custom={1}>
              <Button
                size="lg"
                className="bg-brand text-brand-foreground hover:bg-brand-dark rounded-xl px-9 h-13 text-base font-semibold shadow-lg shadow-brand/25"
                onClick={handleCTA}
              >
                Start Making Notes <ArrowRight className="h-5 w-5 ml-2" />
              </Button>
            </motion.div>
            <motion.p variants={fadeUp} custom={2} className="text-sm text-muted-foreground tracking-wide">
      ...
      <footer className="border-t border-border">
        <div className="max-w-6xl mx-auto px-6 py-10 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-muted-foreground">© {new Date().getFullYear()} NexNotes · Founded by <span className="text-foreground font-medium">Mohit Kala</span></span>
          </div>
        </div>
      </footer>
    </div>
